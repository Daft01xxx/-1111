...
const BOSSES = [
  ...
  { id: 'gold-cat', name: 'GOLD CAT', hp: 500, dmg: 20, reward: 1300, color: '#FFD93D' },
...
export default function BossesPage() {
  ...
  const t =
    language === 'ru'
      ? {
          title: 'Боссы',
          subtitle: 'Прогресс по боссам и наградам',
          reward: 'Награда',
          unlocked: 'Открыт',
          defeated: 'Побеждён',
          locked: 'Закрыт',
  ...
  return (
    <div className={`min-h-screen pb-[calc(108px+env(safe-area-inset-bottom))] ${isDark ? 'bg-[#070707]' : 'bg-[#f7f4ec]'}`}>
      <AppPageHeader
        title={t.title}
        icon={<Skull className="h-5 w-5 text-amber-500" />}
        rightSlot={
          <div
            className={`flex h-10 items-center rounded-xl px-3 text-sm font-bold ${
              isDark ? 'border border-[#8a4e12]/70 bg-[#111111] text-amber-300' : 'border border-amber-300 bg-amber-50 text-amber-700'
            }`}
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className={`mt-1 text-xs ${isDark ? 'text-white/60' : 'text-[#796a4a]'}`}>
                    {t.hp} {boss.hp} • {t.dmg} {boss.dmg} • {t.reward} {boss.reward} 🍺
                  </p>
                </div>
