...
export default function MusicPage() {
  ...
  const forGameLabel = language === 'ru' ? 'Р”Р»СЏ РёРіСЂС‹' : 'For Game'
  const selectedLabel = language === 'ru' ? 'Р’С‹Р±СЂР°РЅРѕ' : 'Selected'
  ...
  const t = {
    title: language === 'ru' ? 'РњСѓР·С‹РєР°' : 'Jukebox',
    yourScore: language === 'ru' ? 'Р’Р°С€ СЂРµРєРѕСЂРґ' : 'Your High Score',
    unlockHint: language === 'ru' ? 'Р Р°Р·Р±Р»РѕРєРёСЂСѓР№С‚Рµ С‚СЂРµРєРё РґРѕСЃС‚РёРіР°СЏ СЂРµРєРѕСЂРґРѕРІ!' : 'Unlock tracks by reaching score milestones!',
    allTracks: language === 'ru' ? 'Р’СЃРµ С‚СЂРµРєРё' : 'All Tracks',
    noTracks: language === 'ru' ? 'РўСЂРµРєРё РїРѕРєР° РЅРµРґРѕСЃС‚СѓРїРЅС‹' : 'No tracks available yet',
    unlockAt: language === 'ru' ? 'РћС‚РєСЂРѕРµС‚СЃСЏ РЅР°' : 'Unlock at',
    free: language === 'ru' ? 'Р‘РµСЃРїР»Р°С‚РЅРѕ' : 'Free',
  }
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#0A0A0B]' : 'bg-orange-50'}`}>
      {currentTrack && (
        <audio
          ref={audioRef}
          src={currentTrack}
      ...
      <div className="p-4 space-y-4">
        <div className={`rounded-xl border p-4 text-center animate-fadeInUp ${
          isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-orange-200'
        }`}>
          <p className={`text-sm ${isDark ? 'text-neutral-400' : 'text-[#6f5b34]'}`}>{t.yourScore}</p>
          <p className="text-2xl font-bold text-orange-500">{highScore.toLocaleString()}</p>
        ...
        {currentTrackData && (
          <div className={`rounded-2xl border p-4 space-y-4 animate-fadeInUp ${
            isDark ? 'bg-neutral-900 border-orange-500/30' : 'bg-white border-orange-300'
            ...
            <div className="space-y-2">
              <input
                ...
                value={currentTime}
                onChange={handleSeek}
                className={`w-full h-2 rounded-full appearance-none cursor-pointer accent-orange-500 ${isDark ? 'bg-neutral-700' : 'bg-orange-200'}`}
              />
            ...
            <div className="flex items-center justify-center gap-4">
              <button
                ...
                className={`p-3 rounded-full transition-colors active:scale-95 ${
                  isDark ? 'bg-neutral-800 text-neutral-400' : 'bg-orange-100 text-[#6f5b34]'
                }`}
              >
              ...
              <button
                ...
                className="p-4 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 text-black active:scale-95 transition-transform"
              >
            ...
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                className={`flex-1 h-2 rounded-full appearance-none cursor-pointer accent-orange-500 ${isDark ? 'bg-neutral-700' : 'bg-orange-200'}`}
              />
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <div className={`rounded-2xl border p-3 space-y-2 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-orange-200'}`}>
            ...
            {battleTracks.map((track) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between gap-3 rounded-xl px-3 py-2 ${isDark ? 'bg-black/20' : 'bg-orange-50 border border-orange-200/80'}`}
                >
                  <button
                    onClick={() =>
                      playTrack({
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${
                      selectedForGame ? 'bg-orange-500 text-black' : isDark ? 'bg-neutral-800 text-neutral-300' : 'bg-orange-100 text-[#694f20]'
                    }`}
                  >
          ...
          {loading ? (
            [...Array(5)].map((_, i) => (
              <div key={i} className={`h-16 rounded-xl animate-pulse ${isDark ? 'bg-neutral-800' : 'bg-orange-100'}`} />
            ))
          ) : tracks.length === 0 ? (
            <div className={`text-center py-8 rounded-xl border ${
              isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-orange-200'
            }`}>
              <Music className={`w-12 h-12 mx-auto mb-3 ${isDark ? 'text-neutral-600' : 'text-orange-400'}`} />
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isCurrentTrack 
                      ? isDark ? 'bg-orange-500/20 border border-orange-500/30' : 'bg-orange-100 border border-orange-300'
                      : unlocked 
                        ? isDark ? 'bg-neutral-900 border border-neutral-800' : 'bg-white border border-orange-200'
                ...
                >
                  <div className={`
                    ...
                    ${isCurrentTrack 
                      ? 'bg-gradient-to-br from-orange-400 to-orange-600' 
                        : isDark ? 'bg-neutral-800' : 'bg-orange-100'
                    }
                  `}>
                    {unlocked ? (
                      isCurrentTrack && isPlaying ? (
                        <div className="flex gap-0.5 items-end h-4">
                          {[0, 1, 2].map((i) => (
                            <div
                              key={i}
                              className={`w-1 rounded-full ${isCurrentTrack ? 'bg-black' : isDark ? 'bg-orange-500' : 'bg-orange-600'}`}
                              style={{
                  ...
                  {!unlocked && (
                    <div className="text-right">
                      ...
                      <p className="text-sm font-bold text-orange-500">{track.unlock_requirement.toLocaleString()}</p>
                    </div>
                  )}
